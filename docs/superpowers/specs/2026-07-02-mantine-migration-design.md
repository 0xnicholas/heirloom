...
每个交互元素都从零手写。例证：
- `SideNav.tsx` 含 18 个 inline SVG 图标组件（~200 行）
...
| `@tabler/icons-react` | 图标库 | `lucide-react` |
...
│   │   │   ├── TopBar.tsx            # → Group + ActionIcon
│   │   │   ├── SideNav.tsx           # → NavLink，删 18 inline SVG
│   │   │   ├── QueryConsole.tsx      # → Drawer 包装 Monaco
...
| `SideNav.tsx`（**250 行**） | `AppShell.Navbar` + `NavLink` + 分组 `Stack` | **删 18 个 inline SVG 组件**（~200 行），改用 Tabler 图标 |
| `QueryConsole.tsx`（155 行） | `Drawer` position=bottom size=50% 包 Monaco | 用 `useDisclosure` 替代 useState；保留 `RECENT_RUNS_KEY` |
...
| `ConfirmDialog.tsx` | **删除**，用 `@mantine/modals` |
...
| Layout 组件 | `src/components/layout/AppLayout.test.tsx` | 2 |
