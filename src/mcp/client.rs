...
use super::transport::{MCPError, MCPTool, MCPToolResult, SseTransport, StdioTransport};
...
impl MCPClient {
    pub async fn from_config(config: &McpClientConfig) -> Result<Self, MCPError> {
        let (mut transport, concurrency) = match config.transport.as_str() {
            "stdio" => {
                let command = config.command.clone().ok_or(MCPError::Transport(
                    "Command required for stdio".to_string(),
                ))?;
...
impl MCPClientManager {
    ...
    pub async fn is_auto_executable(&self, client_name: &str) -> bool {
        let clients = self.clients.read().await;
        clients
            .get(client_name)
    ...
    pub async fn execute_tool(
        ...
        let client = clients
            .get_mut(client_name)
            .ok_or(MCPError::ToolExecution(format!(
                "Client not found: {}",
                client_name
            )))?;
