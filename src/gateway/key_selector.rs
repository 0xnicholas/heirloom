...
pub fn select_key(keys: &[WeightedKey]) -> &WeightedKey {
    ...
    let total_weight: u64 = keys.iter().map(|k| (k.weight * 100.0) as u64).sum();
...
mod tests {
    ...
    fn test_weighted_random_selection() {
        let keys = vec![
            WeightedKey {
                value: SecretString::new("key-a"),
                weight: 1.0,
            },
            WeightedKey {
                value: SecretString::new("key-b"),
                weight: 2.0,
            },
        ];
        ...
        assert!(keys
            .iter()
            .any(|k| k.value.expose() == selected.value.expose()));
    }
    ...
    fn test_weighted_distribution() {
        ...
        for _ in 0..1000 {
            let selected = select_key(&keys);
            *counts
                .entry(selected.value.expose().to_string())
                .or_insert(0) += 1;
        }
