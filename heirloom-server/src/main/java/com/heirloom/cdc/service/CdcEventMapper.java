...
public class CdcEventMapper {
    ...
    public void handleEvent(CdcEvent event, CdcSource source) {
        ...
        switch (event.operation()) {
            ...
            case "UPDATE" -> handleUpdate(event, resourceType, config.stateColumn());
            case "DELETE" -> handleDelete(event, resourceType);
        }
    }
    ...
    private void handleUpdate(CdcEvent event, String resourceType, String explicitStateColumn) {
        ...
        if (stateColumn != null && newFields.containsKey(stateColumn)) {
            ...
            if (newState != null) {
                try {
                    ...
                    log.warn("CDC state transition failed for rid={} to={}: {}", rid, newState, e.getMessage());
                }
            }
    ...
    private void handleDelete(CdcEvent event, String resourceType) {
        String rid = buildDeterministicRid(resourceType, event.oldValues());
        try {
            resourceService.markDeleted(rid);
        } catch (Exception e) {
            log.warn("CDC markDeleted failed for rid={}: {}", rid, e.getMessage());
        }
    }
    ...
    String buildDeterministicRid(String resourceType, Map<String, String> values) {
        ...
        String pkConcat = values.keySet().stream().sorted()
